...
import {formatBarChartData, getSelectedBarTickets, getPanelDataUrl} from '../utils';
...
const OpenDefects = ({portfolios, tickets, brand}) => {
    ...
    const fetchData = () => {
        setIsLoading(true);
        setError(null);
        fetch(getPanelDataUrl(portfolios, 'opendefectspastsla', brand))
            .then((data) => data.json())
            .then((defectsPastSLA) => {
                const rawData = defectsPastSLA.data.openDefectsPastSla;
