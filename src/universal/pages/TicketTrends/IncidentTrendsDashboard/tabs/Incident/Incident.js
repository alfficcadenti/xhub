...
import {getIncidentsData, getIncidentsDataById} from '../../../incidentsHelper';
...
const Incident = () => {
    ...
    const fetchIncident = (id) => {
        fetch(`/v1/incidents/${id}`)
        .then(checkResponse)
        .then( x => setData(x))
    ...
    return (
        <div>
            ...
            <button
                    ...
                    type="button"
                    onClick = {(() => fetchIncident(search))} // calls a function that saves the id in the state
                >
                    {'Search'}
                </button>
            </div>
