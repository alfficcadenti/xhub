...
describe('Incidents component testing', () => {
    ...
    it('passes as props the incidents data to the DataTable', () => {
        ...
        expect(firstRow.TTD.toString().replace(/\s/g, '')).to.be.eql('<divvalue=38>38m</div>');
        expect(firstRow.TTK.toString().replace(/\s/g, '')).to.be.eql('<divvalue=34>34m</div>');
        expect(firstRow.TTF.toString().replace(/\s/g, '')).to.be.eql('<divvalue=28>28m</div>');
        expect(firstRow.TTR.toString().replace(/\s/g, '')).to.be.eql('<divvalue=59>59m</div>');
