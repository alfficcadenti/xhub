Feature: Verify user is able to navigate around vrbo and expedia partner solutions pages
  ...
  Scenario Outline: User is able to use the nav bar to navigate
    ...
    Then the current page title contain <page1>

    Then user clicks on the Availability & Trends Button
    Then user verify availabilityTrendsDropdown exist
    Then user go ahead and clicks on the availabilityTrendsLink3
    Then the current page title contain <page2>
    ...
    Examples:
      | brand                        | page1      | page2         |
      | Vrbo                         | Page Views | Success Rates |
      | ExpediaPartnerSolutions      | FCI        | Page Views    |