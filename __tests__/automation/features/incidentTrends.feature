Feature: Verify user is able to navigate around Incident Trends
  ...
  Scenario Outline: User is able to use the nav bar to navigate
    ...
    Examples:
      ...
      | Expedia                  | platformHealthResiliencyLink2 | false             | pageSizeDropdown | rootCauseOwnerFilter | priorityDropdown      | statusDropdown       | overviewTabContainer | top5TabContainer |                      |                  |
      | Egencia                  | platformHealthResiliencyLink2 | true              |                  |                      | priorityDropdown      | statusDropdown       |                      |                  |                      |                  |
      | Hotelscom                | platformHealthResiliencyLink2 | false             | pageSizeDropdown | rootCauseOwnerFilter | priorityDropdown      | statusDropdown       | overviewTabContainer | top5TabContainer |                      |                  |
      | Vrbo                     | platformHealthResiliencyLink2 | false             |                  |                      | priorityDropdown      | statusDropdown       |                      | top5TabContainer |                      |                  |
